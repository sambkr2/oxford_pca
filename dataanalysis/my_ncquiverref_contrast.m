function my_ncquiverref_contrast(x,y,u,v,units,reftype,refvec,veccol,cont,background,veccenter)
...
%
% background  - CW edit, 2D data at locations of vectors which defines
%               background colour requiring contrast
%
% veccenter -   Sam S edit, add option to allow the starting point of the
%               vector being positioned on the grid point (the same as normal 
%               quiver). If not specified, the default is that all vectors
%               are centered on the grid points.
...
if size(x)~=size(background); error('x and background sizes disagree'); end
if size(y)~=size(background); error('y and background sizes disagree'); end
...
if col
    ...
    % set the colormap to current axis
    hc=colormap(gca,flipud(gray(length(cont))));
    
    for i=1:length(cont)
        
        if i==1
            mask=find(background<cont(i));
        elseif i==length(cont)
            mask=find(background>=cont(i-1));
        else
            mask=find(background<cont(i) & background>=cont(i-1));
        end
        ...
        if strcmp( veccenter, 'quiver' ) % reset to normal quiver 
            xstart=x(mask);
            xend=x(mask)+u;
            ystart=y(mask);
            yend=y(mask)+v;
